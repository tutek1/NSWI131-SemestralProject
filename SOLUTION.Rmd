...
ggplot(speedup_filtered, aes(x = dist)) +
  ...
  labs(title = "Speed-Up: Old version vs New version", x = "Border Distance", y = "Speed-Up (Old/New)", , color = "Texture") +
  theme(
    axis.title = element_text(size = 20),
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.text = element_text(size = 12),
...
The second graph shows a zoomed version of the first one, so we can see the beginning part more clearly. An interesting thing to note here is that the `2048x2048 px` optimised version is almost as fast as the `1024x1024 px` version and even faster for `Border Distance` lower than `0.05`.

The measured speed-up on the third graph for the `2048x2048 px` texture ranges from ~7 to ~13.5 times. The speed-up is not very stable and often dips and peaks, but grows steadily. In my opinion, this is due to the low number of repetitions and the long time it takes to measure the old version, which contributes to the instability due to possible system interruptions, other programs running, etc.

The speed-up for the `1024x1024 px` texture hangs around 5-6 times and does not grow with `Border Distance`. I think this is due to most of the optimisation being done in the distance map calculation, whose compute time does not grow as rapidly as for the `1024x1024 px` texture. Moreover, in these lower generation times, the overhead of dispatching a compute shader and copying textures from CPU to GPU is more noticeable and unavoidable.
...
- One quality of life improvement could be to downscale the texture for generating the preview to `512x512 px` for easy tweaking. Then, during the generation of all normal maps for the sprites of the animations, the user would still have to wait the full time.
- The distance map calculation could first preprocess the texture and save all the edge pixels to a buffer. Then, during the calculation, we could only loop through all the edge pixels. This could further help the performance, but sprites with a lot of "holes" (`alpha=0` pixels) could perform worse.
- I could use a smarter distance map calculation algorithm instead of the simple brute-force "loop over a square" method. I found that I could use the Jump Flooding Algorithm (JFA) or vector propagation algorithms, which would allow me to find the nearest empty pixel in as little as $O(log\ n)$.
- The last thing I thought of is to use a multipass shader. Each pass would expand the edge pixels inwards by one pixel. However, for large textures and big `Border Distance` values, I would need hundreds of passes, and the distance map would need to be synchronised between every pass. Synchronisation could be done by having each pass be a dispatch, but I think that this would perform even worse than the old version due to the overhead from the numerous dispatches.
